<template>
  <div class="results-screen p-4 sm:p-6 bg-gray-100 min-h-screen">
    <div class="max-w-6xl mx-auto">
      <h2 class="text-3xl font-bold mb-8 text-center text-gray-800">{{ $t('recentGames') }}</h2>
      ...
      <TransitionGroup name="list" tag="div" class="game-list space-y-4">
        ...
        >
          ...
          <p class="text-lg font-medium mb-4">
            <span class="text-blue-600">{{ $t('ourTeam') }}:</span> {{ getGameScore(game).team }} - 
  ...
  setup() {
    ...
    const fetchGames = async () => {
      ...
      try {
        ...
        error.value = t('failedToLoadGames');
      } finally {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          try {
            ...
            alert(t('gamesImportedSuccessfully'));
          } catch (error) {
            console.error('Error importing games:', error);
            alert(t('errorImportingGames'));
          }
        };
